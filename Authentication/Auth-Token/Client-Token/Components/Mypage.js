...
class Mypage extends Component {
  ...
  async accessTokenRequest() {
   ...
   const result = await axios.get(
       'https://localhost:4000/accesstokenrequest',
   ...
   if (result.message === 'ok') {
    ...
    //     const message = "Access token is expired. Please use refresh token.";
    //     return this.setState({email: message, createdAt: message})
    //   }
    //   const {userId, email, createdAt} = data.data.userInfo
...
if (result.message === 'ok') {
  ...
  this.props.issueAccessToken(result.data.accessToken);
}
  ...
  //   console.log(data);
  //   if(data.message !== 'ok') {
  //     const message = "Refresh token is expired. Please login again.";
  //     return this.setState({email: message, createdAt: message})
  //   }
  //   const {userId, email, createdAt} = data.data.userInfo
  //   this.setState({userId, email, createdAt})
  //   this.props.issueAccessToken(data.data.accessToken)
  // })
  }
